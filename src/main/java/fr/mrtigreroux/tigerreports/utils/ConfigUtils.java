...
public class ConfigUtils {
	...
	public static boolean isPlaceholderUsedInCommands(ConfigurationSection config, String path, String placeholder) {
	...
	public static void processCommands(ConfigurationSection config, String path, Report r, Player staff,
	        String staffReason, VaultManager vm, BungeeManager bm) {
		if (staff == null) {
			Logger.MAIN.warn(() -> ConfigUtils.getInfoMessage(
			        "Could not process commands at <" + path + "> for report #" + r.getId()
			                + " because staff is unknown.",
			        "Les commandes configurees dans <" + path + "> pour le signalement #" + r.getId()
