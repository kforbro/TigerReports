...
public class BungeeManager implements PluginMessageListener {
    ...
    private final List<ResultCallback<String>> localServerNameResultCallbacks = new ArrayList<>();
    ...
    private static class KnownServer {
        ...
        private final List<Notification> unsentNonEphemeralNotifications = new CollectionUtils.LimitedOrderedList<>(
                MAX_OFFLINE_SERVER_KEPT_UNSENT_NON_EPHEMERAL_NOTIFICATIONS);
        private final List<Notification> unsentEphemeralNotifications = new CollectionUtils.LimitedOrderedList<>(
    ...
    private final Map<String, KnownServer> knownServers = new HashMap<>();
    /**
     * player name -> server name (null if unknown server name but the player is online)
     */
    private final Map<String, String> onlinePlayersServer = new HashMap<>();
     ...
     * NB: The notification time will be the one of sending, and not the one of the creation of the notification.
     */
    private final List<UnsentNotification> unsentNotificationsWhenServersUnknown = new CollectionUtils.LimitedOrderedList<>(
     ...
     */
    private final Map<String, TeleportLocationNotification> playersTeleportLocNotif = new HashMap<>();
            ...
            if (playerOfflineNotBroadcastedUUID != p.getUniqueId()) {
                ...
                getServerName((localServerName) -> {
                    sendPlayerOnlineNotification(playerOfflineNotBroadcastedName, false, localServerName,
                            curPlayerOfflineNotBroadcastedUUID, null);
                });
