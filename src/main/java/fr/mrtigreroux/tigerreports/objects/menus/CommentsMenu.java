...
public class CommentsMenu extends ReportManagerMenu implements UpdatedMenu, ReportsManager.ReportCommentsPageListener {
    ...
    public CommentsMenu(User u, int page, int reportId, ReportsManager rm, Database db, TaskScheduler taskScheduler,
    ...
    public void onUpdate(Inventory inv) {
        ...
        inv.setItem(size - 7, page >= 2 ? MenuItem.PAGE_SWITCH_PREVIOUS.get() : MenuRawItem.GUI.create());

        inv.setItem(size - 3,
                rm.isCachedReportCommentsPageNotEmpty(reportId, page + 1) ? MenuItem.PAGE_SWITCH_NEXT.get()
